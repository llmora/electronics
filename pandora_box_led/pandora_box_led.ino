 ...
 * 
 * The LED strip "data in" (DI) pin should be tied to pin PB4 in the ATtiny85 and the switch should be tied to pin PB0 (when the switch is open the LEDs switch on)
 * 
...
#define NUM_LEDS 3 // Adjust to the number of leds on your strip - too many leds will require quite a bit of current, make sure your power supply can handle it. We used 3 WS2811 LEDs and powered from a 600mAh 1-cell 3.7V battery
...
void setup() {
    pinMode(SWITCH_PIN, INPUT_PULLUP); // Enable pull-up resistor on switch pin - when the switch is open the pin goes to HIGH. when it is closed it goes to LOW
...
void loop() {
  ...
  if(switchEnabled()) {
    ...
    // If the switch is not enabled, switch off the LEDs (who can tell if the LEDs are off when you can't see them!!!)
    switchOff();
    fadeShown = 0;
  }

  delay(500); // Save battery in exchange of not-immediate response, a good trade-off as we are aiming for long battery life
}
